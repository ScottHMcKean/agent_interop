# Agent Dock: A2A + MCP Registry

**Agent Dock** sits alongside the Lakehouse/Lakebase to give agents a place to land, discover each other, and exchange tasks and information. It uses Model Context Protocol (MCP) as the registry surface and A2A for runtime agent-to-agent communication.

## Table of Contents

- [Agent Dock: A2A + MCP Registry](#agent-dock-a2a--mcp-registry)
...
```sh
bash samples/python/agents/agent_dock/run.sh
```

1. Start the MCP Server:

   ```sh
   cd samples/python/agents/agent_dock
   uv venv # (if not already done)
   source .venv/bin/activate
   # Runs on port 10100 by default, change as needed by setting the --host and --port parameters.
   uv run  --env-file .env agent-dock --run mcp-server --transport sse
   ```
...
- **`src/agent_dock/`**: The primary Python source code for this A2A with MCP sample.
